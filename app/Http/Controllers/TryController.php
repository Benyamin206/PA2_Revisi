...
class TryController extends Controller
{
    ...
    //         }
    //     }
    //     return view('Pesan_Jadwal.index', compact('jadwals', 'date'));
    // }
    ...
    public function filterJadwal(Request $request)
    {
        ...
        $jadwals = Jadwal::whereDate('waktu_berangkat', $date)->get();
        
        return view('jadwal.index_pesan', compact('jadwals'));
    }

    public function index_jadwal_cari_pergi_penumpang_filter(Request $request){
        $request->validate([
            'date' => 'required|date_format:Y-m-d',
            'rute' => 'required'
        ]);
        ...
        $Njadwals = Jadwal::with('rute', 'kapal', 'supir')
        ->where('rute_id', $rute)
        ->whereDate('waktu_berangkat', $date)->get();
