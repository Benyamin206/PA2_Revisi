...
<table class="table table-image">
    <thead>
      <tr>
        ...
        <th scope="col">Tanggal Tersedia Booking</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$k->nama}}</td>
            <td>{{$k->deskripsi}}</td>
            {{-- <td><img src="{{ url('storage/Kapal_Image/1714672834adshj.png') }}
              " alt="{{$k->gambar}}" height="100" width="100"></td> --}}
              <td><img src="{{ asset('storage/Kapal_Image/'.$k->gambar) }}
                " alt="{{$k->gambar}}" height="100" width="100"><br>{{$k->gambar}}</td>
            <td>{{$k->kapasitas}}</td>
            <td>{{$k->aktif ? 'Aktif' : 'Tidak Aktif'}}</td>
            <td>{{$k->available_booking ? 'Ya' : 'Tidak'}}</td>
            <td>{{$k->booking ? 'Sedang Dibooking' : 'Free'}}</td>
            <td>{{$k->tanggal_tersedia_booking}}</td>
            <td>
