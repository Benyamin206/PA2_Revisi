    ...
    <div class="ticket-body">
      <center>
        <div class="ticket-details">
          ...
          <p><strong>Kode Pemesanan :</strong> {{$order->id}}</p>
...
@if($order->jadwal_pulang_id != NULL)
  <div class="cargo-details">
    <h5>Detail Muatan Pulang</h5>
    @php
      
    @endphp
    @foreach ($order->detail_pesan_jadwal_pulang as $dpjp)
    <p><strong>{{$dpjp->muatan->nama}}</strong> :  {{$dpjp->jumlah}} (Harga per item =>{{ 'Rp.' . number_format($dpjp->muatan->harga_per_item, 0, ',', '.') }}
      )</p> 
    @php
      $totalHarga += $dpjp->jumlah * $dpjp->muatan->harga_per_item;
    @endphp
    @endforeach
  </div>
@endif
